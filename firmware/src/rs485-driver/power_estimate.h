...
static inline int64_t hisense_voltage_mv(uint8_t v50) { return (int64_t) v50 * 1000; }
...
static inline void hisense_energy_add(uint64_t *acc_mw_ms, int64_t power_mw, uint32_t dt_ms) {
  if (power_mw > 0)
    *acc_mw_ms += (uint64_t) power_mw * (uint64_t) dt_ms;
}

static inline uint64_t hisense_energy_mwh(uint64_t acc_mw_ms) { return acc_mw_ms / 3600000ULL; /* mW*ms -> mWh */ }
