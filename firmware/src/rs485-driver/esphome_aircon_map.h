...
static inline bool esphome_mode_to_hisense(uint8_t climate_mode, HisenseMode *out) {
  switch (climate_mode) {
    ...
    case ESPHOME_CLIMATE_MODE_COOL:
      *out = HISENSE_MODE_COOL;
      return true;
    case ESPHOME_CLIMATE_MODE_HEAT:
      *out = HISENSE_MODE_HEAT;
      return true;
    case ESPHOME_CLIMATE_MODE_FAN_ONLY:
      *out = HISENSE_MODE_FAN;
      return true;
    case ESPHOME_CLIMATE_MODE_DRY:
...
static inline uint8_t hisense_mode_to_esphome(HisenseMode m) {
  switch (m) {
    case HISENSE_MODE_FAN:
      return ESPHOME_CLIMATE_MODE_FAN_ONLY;
    case HISENSE_MODE_HEAT:
      return ESPHOME_CLIMATE_MODE_HEAT;
...
static inline uint8_t hisense_to_climate_action(bool power_on, HisenseMode mode, uint8_t comp_freq) {
  uint16_t rs;
  if (!power_on)
    return ESPHOME_CLIMATE_ACTION_OFF;
  if (mode == HISENSE_MODE_DRY)
    return comp_freq > 0 ? ESPHOME_CLIMATE_ACTION_DRYING : ESPHOME_CLIMATE_ACTION_IDLE;
  rs = hisense_to_running_state(power_on, mode, comp_freq);
  switch (rs) {
    case 1:
      return ESPHOME_CLIMATE_ACTION_HEATING;
    case 2:
      return ESPHOME_CLIMATE_ACTION_COOLING;
    case 4:
      return ESPHOME_CLIMATE_ACTION_FAN;
    default:
      return ESPHOME_CLIMATE_ACTION_IDLE; /* powered but not working */
  }
}

static inline uint8_t hisense_to_climate_swing(bool vswing_on, bool hswing_on) {
  if (vswing_on && hswing_on)
    return ESPHOME_CLIMATE_SWING_BOTH;
  if (vswing_on)
    return ESPHOME_CLIMATE_SWING_VERTICAL;
  if (hswing_on)
    return ESPHOME_CLIMATE_SWING_HORIZONTAL;
  return ESPHOME_CLIMATE_SWING_OFF;
}

static inline void climate_swing_to_hisense(uint8_t swing, HisenseSwingMode *vswing, HisenseSwingMode *hswing) {
...
static inline uint8_t esphome_fan_enum_to_index(uint8_t fan_mode) {
  switch (fan_mode) {
    case ESPHOME_CLIMATE_FAN_QUIET:
      return 1;
    case ESPHOME_CLIMATE_FAN_LOW:
      return 2;
    case ESPHOME_CLIMATE_FAN_MIDDLE:
      return 3;
    case ESPHOME_CLIMATE_FAN_MEDIUM:
      return 4;
    case ESPHOME_CLIMATE_FAN_FOCUS:
      return 5;
    case ESPHOME_CLIMATE_FAN_HIGH:
    case ESPHOME_CLIMATE_FAN_ON:
      return 6;
    default:
      return ESPHOME_FAN_INDEX_AUTO;
  }
}
 ...
 * list. */
static inline uint8_t esphome_fan_published_index(uint8_t idx) { return idx == 1 ? 2 : idx; }

static inline HisenseFanSpeed esphome_fan_index_to_hisense(uint8_t idx) {
  if (idx == ESPHOME_FAN_INDEX_AUTO)
    return HISENSE_FAN_AUTO;
...
static inline bool esphome_sync_shadow_setpoint(int8_t setpoint_c, bool temp_unit_f, HisenseCommand *cmd) {
  int8_t wire;
  if (!hisense_shadow_setpoint_from_status(setpoint_c, temp_unit_f, &wire))
    return false;
  cmd->setpoint = wire;
...
static inline HisenseFeature hisense_feature_from_status(bool eco_on, bool turbo_on) {
  if (eco_on)
    return HISENSE_FEATURE_ECO;
  if (turbo_on)
    return HISENSE_FEATURE_TURBO;
...
static inline bool esphome_preset_available(uint8_t idx, uint8_t support) {
  ...
  if (idx >= ESPHOME_PRESET_COUNT)
    return false;
  r = &k_esphome_presets[idx];
  if (r->eco && !(support & ESPHOME_SUPPORT_ECO))
    return false;
  if (r->mute && !(support & ESPHOME_SUPPORT_QUIET))
    return false;
  if (r->turbo && !(support & ESPHOME_SUPPORT_TURBO))
    return false;
  if (r->sleep && !(support & ESPHOME_SUPPORT_SLEEP))
    return false;
  return true;
}
...
static inline HisenseSpecialState hisense_special_from_status(bool eco_on, bool turbo_on, bool mute_on,
                                                              uint8_t sleep_raw) {
...
static inline uint8_t esphome_preset_plan(const HisenseSpecialState *now, uint8_t target,
  ...
  if (target >= ESPHOME_PRESET_COUNT)
    return 0;
  ...
  if (t->sleep == 0 && now->sleep != 0) {
    out[n].kind = HISENSE_SPECIAL_OP_SLEEP;
    out[n].value = 0;
    n++;
  }
  ...
  if (now->mute && !t->mute) {
    out[n].kind = HISENSE_SPECIAL_OP_MUTE;
    out[n].value = 0;
    n++;
  }
  if (feature_set) {
