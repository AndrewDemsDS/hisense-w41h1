...
static const HisenseFanRow k_hisense_fan_table[] = {
    {0x02, 1, 10, HISENSE_FAN_QUIET}, {0x0A, 2, 25, HISENSE_FAN_LOW},      {0x0C, 3, 42, HISENSE_FAN_MED_LOW},
    {0x0E, 4, 58, HISENSE_FAN_MID},   {0x10, 5, 75, HISENSE_FAN_MED_HIGH}, {0x12, 6, 100, HISENSE_FAN_HIGH},
};
...
static inline HisenseFanSpeed percent_to_hisense_fan(uint8_t pct) {
  ...
  if (pct <= 16)
    return HISENSE_FAN_QUIET;
  if (pct <= 33)
    return HISENSE_FAN_LOW;
  if (pct <= 50)
    return HISENSE_FAN_MED_LOW;
  if (pct <= 67)
    return HISENSE_FAN_MID;
  if (pct <= 83)
    return HISENSE_FAN_MED_HIGH;
...
static inline HisenseFanSpeed fanmode_to_hisense_fan(uint8_t fanmode) {
  switch (fanmode) {
    ...
    case 4:
      return HISENSE_FAN_HIGH;  // On -> full speed
    default:
      return HISENSE_FAN_AUTO;  // Off/Auto/Smart -> auto (A/C fan can't idle off)
  }
}
...
static inline void matter_echo_note(MatterEchoLedger *l, uint8_t v) {
  ...
  if (l->len >= MATTER_ECHO_LEDGER_LEN) {
    for (i = 1; i < MATTER_ECHO_LEDGER_LEN; i++)
      l->vals[i - 1] = l->vals[i];
...
static inline bool matter_echo_consume(MatterEchoLedger *l, uint8_t v) {
  ...
  for (i = 0; i < l->len; i++) {
    if (l->vals[i] == v) {
      drop = i + 1;
      for (j = drop; j < l->len; j++)
        l->vals[j - drop] = l->vals[j];
...
static inline uint8_t hisense_fan_raw_to_percent(uint8_t raw) {
  if (raw == HISENSE_FAN_RAW_AUTO)
    return HISENSE_FAN_AUTO_PERCENT;  // AUTO -> non-zero, not off
  const HisenseFanRow *r = hisense_fan_row_by_raw(raw);
...
static inline HisenseFanSpeed hisense_fan_raw_to_cmd(uint8_t raw) {
  if (raw == HISENSE_FAN_RAW_AUTO)
    return HISENSE_FAN_AUTO;  // 0x01 = genuine AUTO
  const HisenseFanRow *r = hisense_fan_row_by_raw(raw);
...
static inline uint8_t hisense_fan_raw_to_fanmode(uint8_t raw, bool power_on) {
  ...
  const HisenseFanRow *r = hisense_fan_row_by_raw(raw);
  if (!r)
    return 5;                             // 0x01 auto / unknown-but-running -> Auto
...
static inline uint16_t hisense_to_running_state(bool power_on, HisenseMode mode, uint8_t comp_freq) {
  if (!power_on)
    return 0;
  if (mode == HISENSE_MODE_FAN)
    return 4;  // Fan
  if (comp_freq > 0)
    return (mode == HISENSE_MODE_HEAT) ? 1 : 2;  // Heat : Cool
...
static inline int matter_round_setpoint_c(int16_t centi) { return (int) ((centi + (centi >= 0 ? 50 : -50)) / 100); }
...
static inline bool matter_gate_eco(const HisenseFeatures *f) { return (f == NULL) || !f->valid || f->power_save; }

/* Quiet switch (ep4) <- ac_fan_mute. */
static inline bool matter_gate_quiet(const HisenseFeatures *f) { return (f == NULL) || !f->valid || f->fan_mute; }
...
static inline uint32_t matter_thermostat_featuremap(const HisenseFeatures *f) {
  if (f == NULL || !f->valid)
    return MATTER_THERMOSTAT_FEATUREMAP_FULL;
