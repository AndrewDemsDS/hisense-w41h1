...
bool hisense_status_checksum_ok(const uint8_t *f, size_t n) {
  if (n < 6)
    return false;  // F4 F5 + chk_hi chk_lo + F4 FB minimum
  if (f[n - 2] != HISENSE_ETX1 || f[n - 1] != HISENSE_ETX2)
    return false;          // end tag (defensive)
...
void hisense_set_features_cb(hisense_features_cb_t cb) { s_features_cb = cb; }
...
static size_t build_command_impl(const HisenseCommand *cmd, uint8_t *out, size_t out_cap, int ovr_off,
                                 uint8_t ovr_val) {
  ...
  uint8_t hswing_bits = (cmd->hswing == HISENSE_SWING_SWING)       ? 0x3
                        : (cmd->hswing == HISENSE_SWING_DIRECTION) ? 0x1
                                                                   : 0x0;
  ...
  switch (cmd->display) {
    case HISENSE_DISPLAY_ON:
      frame[36] = 0xC0;
      break;
    case HISENSE_DISPLAY_OFF:
      frame[36] = 0x40;
      break;
...
bool hisense_devtype_from_reply(const uint8_t *reply, size_t n, uint8_t *hi, uint8_t *lo) {
  ...
  if (hi)
    *hi = reply[16];  // inner[3] = device-type code  -> stock 0x10009687
  if (lo)
    *lo = reply[17];  // inner[4] = sub-type          -> stock 0x10009686
  return true;
}
...
static bool hisense_consume_status(size_t n) {
  if (n >= HISENSE_STATUS_HEADER_LEN && s_msg_buf[13] == 0x66) {
    ...
    if (!hisense_status_checksum_ok(s_msg_buf, n)) {
      s_chk_mismatch++;
      return false;
    }
...
void hisense_recommission_window_closed(void) {
  s_smartcfg_lockout_until = xTaskGetTickCount() + pdMS_TO_TICKS(HISENSE_SMARTCFG_REENTRY_LOCKOUT_MS);
}
...
void hisense_set_link_cb(hisense_link_cb_t cb) { s_link_cb = cb; }
...
static void hisense_check_link_reply(size_t n) {
  ...
  {  // snapshot the whole reply for bench diffing (see s_last_link_frame)
    ...
    for (size_t i = 0; i < cp; i++)
      s_last_link_frame[i] = s_msg_buf[i];
    s_last_link_frame_len = (uint8_t) cp;
    if (s_link_frame_cb != NULL)
      s_link_frame_cb(s_last_link_frame, s_last_link_frame_len);
  }
  ...
  uint8_t req_gated = hisense_smartcfg_lockout_mask(req, xTaskGetTickCount(), s_smartcfg_lockout_until);
  ...
  int edge = hisense_recommission_debounce(req_gated, &s_link_req_streak, &s_recommission_latched, hold);
  if (edge > 0) {
...
void hisense_set_link_frame_cb(hisense_link_frame_cb_t cb) { s_link_frame_cb = cb; }
...
uint8_t hisense_get_last_link_frame(uint8_t *out, uint8_t cap) {
  ...
  for (uint8_t i = 0; i < n; i++)
    out[i] = s_last_link_frame[i];
  return n;
}

uint8_t hisense_get_last_link_req(void) { return s_last_link_req; }

void hisense_set_recommission_cancel_cb(hisense_recommission_cancel_cb_t cb) { s_recommission_cancel_cb = cb; }

void hisense_set_recommission_cb(hisense_recommission_cb_t cb) { s_recommission_cb = cb; }

void hisense_set_provisioning(bool on) {
  s_prov_active = on;  // see the header doc for the "77" prov_status semantics
}

bool hisense_get_provisioning(void) { return s_prov_active; }
...
static void hisense_bus_task(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (++pt_poll >= HISENSE_PRODUCTTYPE_POLL_CYCLES)
      pt_poll = 0;
...
int hisense_init(hisense_status_cb_t cb) {
  ...
  BaseType_t ok = xTaskCreate(hisense_bus_task, "hisense_bus", HISENSE_BUS_TASK_STACK, NULL, tskIDLE_PRIORITY + 2,
                              &s_bus_task_handle);
  if (ok != pdPASS) {
    s_initialized = false;
