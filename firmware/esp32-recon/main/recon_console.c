...
void recon_watch_clear_if(FILE *sink) {
  ...
  if (s_watch == sink)
    s_watch = NULL;
  if (s_verify.armed && s_verify.out == sink)
    s_verify.armed = false;
  UNLOCK();
}
...
void recon_watch_note_raw(const char *label, const uint8_t *buf, int len) {
  LOCK();
  if (s_watch) {
    fprintf(s_watch, "[%s] dir=0x%02x class=0x%02x len=%d:", label, len > 2 ? buf[2] : 0, len > 13 ? buf[13] : 0, len);
    if (s_watch_hex)
      for (int i = 0; i < len; i++)
        fprintf(s_watch, " %02x", buf[i]);
    fprintf(s_watch, "\r\n");
    fflush(s_watch);
  }
  UNLOCK();
...
void recon_note_raw(int kind, const uint8_t *buf, int len) {
  if (kind < 0 || kind >= RAW_KINDS || len <= 0)
    return;
  if (len > RAW_MAX)
    len = RAW_MAX;
  LOCK();
...
static bool is_master(FILE *out) {
  if (recon_mode_get() == RECON_MODE_MASTER)
    return true;
...
static int cmd_mode(int argc, char **argv) {
  ...
  if (!strcmp(argv[1], "tap"))
    recon_mode_set_and_reboot(RECON_MODE_TAP);
  else if (!strcmp(argv[1], "master"))
    recon_mode_set_and_reboot(RECON_MODE_MASTER);
  else {
    printf("usage: mode [tap|master]\r\n");
    return 1;
  }
...
static int cmd_stats(int argc, char **argv) {
  ...
  printf("mode=%s frames=%u chkfail=%u cmd_frames=%u | HAL tx=%u rx=%u "
         ...
         recon_mode_str(recon_mode_get()), (unsigned) s.frames, (unsigned) s.chkfail, (unsigned) s.cmd_frames,
         (unsigned) g_hal_tx_bytes, (unsigned) g_hal_rx_bytes, (unsigned) g_hal_rx_task_up, (unsigned) g_hal_evt_total,
         (unsigned) g_hal_evt_data);
  return 0;
}
...
static HisenseCommand s_cmd = {
    HISENSE_MODE_COOL, 24, false, HISENSE_FAN_AUTO, HISENSE_SWING_OFF, HISENSE_SWING_OFF, HISENSE_FEATURE_NONE, false};

static int fan_expected_raw(HisenseFanSpeed f) {
  switch (f) {  // status raw (even) vs command (odd); see header
    case HISENSE_FAN_AUTO:
      return 0x01;
    case HISENSE_FAN_QUIET:
      return 0x02;
    case HISENSE_FAN_LOW:
      return 0x0A;
    case HISENSE_FAN_MED_LOW:
      return 0x0C;
    case HISENSE_FAN_MID:
      return 0x0E;
    case HISENSE_FAN_MED_HIGH:
      return 0x10;
    case HISENSE_FAN_HIGH:
      return 0x12;
    default:
      return -1;
  }
...
static int cmd_set(int argc, char **argv) {
  ...
  if (!strcmp(k, "temp")) {
    ...
    if (!strcmp(v, "cool"))
      m = HISENSE_MODE_COOL;
    else if (!strcmp(v, "heat"))
      m = HISENSE_MODE_HEAT;
    else if (!strcmp(v, "dry"))
      m = HISENSE_MODE_DRY;
    else if (!strcmp(v, "fan"))
      m = HISENSE_MODE_FAN;
    ...
    if (!strcmp(v, "auto"))
      ...
      fs = HISENSE_FAN_QUIET;
    else if (!strcmp(v, "low"))
      fs = HISENSE_FAN_LOW;
    else if (!strcmp(v, "medlow"))
      fs = HISENSE_FAN_MED_LOW;
    else if (!strcmp(v, "mid"))
      fs = HISENSE_FAN_MID;
    else if (!strcmp(v, "medhigh"))
      fs = HISENSE_FAN_MED_HIGH;
  ...
  if (!n) {
    printf("build failed (bad value / buffer)\r\n");
    return 1;
  }
  ...
  if (vfield)
    recon_verify_arm(stdout, vfield, vexp);
  return 0;
}
...
static int cmd_producttype(int argc, char **argv) {
  ...
  if (hisense_get_features(&ft) && ft.valid) {
    printf("features(last): cool_heat=%d ai=%d inf_fan=%d power_save=%d fan_mute=%d "
           ...
           ft.cool_heat, ft.ai, ft.infinite_fan, ft.power_save, ft.fan_mute, ft.swing_dir_8, ft.purify, ft.humidity,
           ft.power_display, ft.demand_resp);
  } else {
...
static int cmd_raw(int argc, char **argv) {
  if (argc < 2) {
    printf("usage: raw <hexbytes>   (master; sends a literal frame)\r\n");
    return 1;
  }
  if (!is_master(stdout))
    return 1;
  ...
  if (!hisense_send_frame(f, (size_t) n)) {
    printf("send queue full\r\n");
    return 1;
  }
...
static int cmd_decode(int argc, char **argv) {
  if (argc < 2) {
    printf("usage: decode <hexbytes>   (offline parse; any mode)\r\n");
    return 1;
  }
  uint8_t f[256];
  char joined[600];
...
static int cmd_snap(int argc, char **argv) {
  ...
  if (n > 0) {
    memcpy(s_raw_base[k], s_raw_last[k], n);
    s_raw_base_len[k] = n;
  }
  UNLOCK();
  if (n > 0)
    printf("snapped %s baseline (%d bytes). toggle a feature, then `diff %s`.\r\n", k ? "cmd" : "status", n,
           k ? "cmd" : "status");
  else
    printf("no %s frame captured yet (tap mode captures raw frames)\r\n", k ? "cmd" : "status");
  return 0;
}
...
static int cmd_diff(int argc, char **argv) {
  ...
  UNLOCK();
  if (bl <= 0) {
    printf("no baseline — run `snap %s` first\r\n", k ? "cmd" : "status");
    return 1;
  }
  if (cl <= 0) {
    printf("no current %s frame captured\r\n", k ? "cmd" : "status");
    return 1;
  }
  if (bl != cl)
    printf("note: length changed %d -> %d\r\n", bl, cl);
...
static int cmd_token(int argc, char **argv) {
  ...
  printf("link session token (reply [9]/[10]): %02x %02x  [%s]\r\n", hi, lo,
         seen ? "captured" : "seed — no reply captured");
  if (!seen) {
...
static int cmd_wifi(int argc, char **argv) {
  ...
  if (argc >= 2 && !strcmp(argv[1], "status")) {
    recon_wifi_status(stdout);
    return 0;
  }
  if (argc < 3) {
    printf("usage: wifi <ssid> <pass> | wifi clear | wifi status\r\n");
    return 1;
  }
