...
static const uint8_t STATUS_HDR[16] = {0xF4, 0xF5, 0x01, 0x40, 0x97, 0x01, 0x00, 0xFE,
...
static int make_status(uint8_t *f, int mode_nibble, int run, int set_c, int indoor_c, uint8_t fan_raw, int outdoor_c,
                       uint8_t flags1, uint8_t flags2, uint8_t comp, uint8_t sleep_raw, int coil_c) {
  memset(f, 0, 160);
...
int recon_selftest(FILE *out) {
  ...
  struct {
    ...
    uint8_t exp;
      ...
      {HISENSE_MODE_AUTO, 0x90}, {HISENSE_MODE_COOL, 0x50}, {HISENSE_MODE_HEAT, 0x30},
      {HISENSE_MODE_DRY, 0x70},  {HISENSE_MODE_FAN, 0x10},
  };
  ...
  struct {
    ...
    uint8_t exp;
      ...
      {HISENSE_FAN_AUTO, 0x01}, {HISENSE_FAN_QUIET, 0x03},    {HISENSE_FAN_LOW, 0x0B},  {HISENSE_FAN_MED_LOW, 0x0D},
      {HISENSE_FAN_MID, 0x0F},  {HISENSE_FAN_MED_HIGH, 0x11}, {HISENSE_FAN_HIGH, 0x13},
  };
  ...
  c.feature = HISENSE_FEATURE_ECO;
  n = hisense_build_command(&c, f, sizeof(f));
  CHECK(n && f[33] == 0x30, "cmd eco -> byte[33]=0x30");
  c.feature = HISENSE_FEATURE_TURBO;
  n = hisense_build_command(&c, f, sizeof(f));
  ...
  CHECK(n >= 50 && f[46] == 0x01 && f[47] == 0xDF && f[48] == 0xF4 && f[49] == 0xFB, "power ON tail 01 DF F4 FB");
  n = hisense_build_power_frame(false, f, sizeof(f));
  CHECK(n >= 50 && f[46] == 0x02 && f[47] == 0x31 && f[48] == 0xF4 && f[49] == 0xFB, "power OFF tail 02 31 F4 FB");

  // ---- mute / sleep single-field frames ----
  n = hisense_build_mute_frame(true, f, sizeof(f));
  CHECK(n && f[35] == 0x30, "mute on byte[35]=0x30");
  n = hisense_build_mute_frame(false, f, sizeof(f));
  CHECK(n && f[35] == 0x10, "mute off byte[35]=0x10");
  for (int p = 0; p <= 4; p++) {
  ...
  CHECK(n == 21 && f[13] == 0x66 && f[14] == 0x40 && f[17] == 0x01 && f[18] == 0xF3 && f[19] == 0xF4 && f[20] == 0xFB,
        "producttype request bytes");

  // ---- status parse: field decode ----
  uint8_t s[160];
  HisenseState st;
  make_status(s, HISENSE_MODE_COOL, 1, 22, 21, 0x01, 32, 0, 0, 0, 0, 0);
  CHECK(hisense_parse_status(s, 160, &st) && st.valid && st.power_on && st.mode == HISENSE_MODE_COOL &&
            st.setpoint_c == 22 && st.indoor_temp_c == 21 && st.outdoor_temp_c == 32 && st.fan_raw == 0x01,
        "status decode A (cool/22/21/32)");

  make_status(s, HISENSE_MODE_COOL, 1, 22, 21, 0x12, 32, 0x86, 0x04, 55, 0x04, 27);
  CHECK(hisense_parse_status(s, 160, &st) && st.valid && st.vswing_on && st.eco_on && st.turbo_on && st.mute_on &&
            st.fan_raw == 0x12 && st.compressor_freq == 55 && st.sleep_on && st.sleep_raw == 0x04 &&
            st.coil_temp_c == 27,
  ...
  CHECK(hisense_parse_status(s, 160, &st) && st.valid && st.mode == HISENSE_MODE_AUTO, "status mode nibble 5 -> AUTO");
