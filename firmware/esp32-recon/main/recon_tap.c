...
static void classify(const uint8_t *f, int len) {
  ...
  if (dir == 0x00) {
    recon_note_cmd_frame();
    recon_note_raw(1, f, len);
  }  // module->A/C
...
static void feed(uint8_t b) {
  switch (s_state) {
    ...
    case S_BODY:
      if (b == 0xF4)
        s_state = S_BODY_F4;  // pending: stuffed F4 or end tag
      else if (s_len < (int) sizeof(s_buf))
        s_buf[s_len++] = b;
      else
        s_state = S_IDLE;  // overflow -> resync
      break;
    case S_BODY_F4:
      if (b == 0xFB) {  // F4 FB = end
...
void recon_tap_start(void) {
  ...
  ESP_ERROR_CHECK(uart_set_pin(TAP_UART, TAP_TX_PIN, TAP_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
