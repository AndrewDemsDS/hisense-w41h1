...
static HisenseCommand s_cmd = {
    HISENSE_MODE_COOL,       24, false, HISENSE_FAN_AUTO, HISENSE_SWING_OFF, HISENSE_SWING_OFF, HISENSE_FEATURE_NONE,
...
const ModeOpt kSleepModes[] = {
    {chip::CharSpan::fromCharString("Off"), 0, {}},  {chip::CharSpan::fromCharString("General"), 1, {}},
    {chip::CharSpan::fromCharString("Old"), 2, {}},  {chip::CharSpan::fromCharString("Young"), 3, {}},
...
class SleepModesMgr : public chip::app::Clusters::ModeSelect::SupportedModesManager {
 public:
  ...
  chip::Protocols::InteractionModel::Status getModeOptionByMode(chip::EndpointId, uint8_t mode,
                                                                const ModeOpt **out) const override {
...
static void arm_sync_hold() {
  s_sync_hold_until =
      chip::System::SystemClock().GetMonotonicTimestamp() + chip::System::Clock::Milliseconds32(HISENSE_SYNC_HOLD_MS);
}
static void flush_cmd() {
  ...
  if (!n) {
    ESP_LOGE(TAG,
             "combined command REJECTED by the builder (mode=%d setpoint=%d "
...
extern "C" void diag_get_cmd_state(int *mode, int *setpoint, int *fahrenheit) {
  if (mode)
    *mode = (int) s_cmd.mode;
  if (setpoint)
    *setpoint = (int) s_cmd.setpoint;
  if (fahrenheit)
    *fahrenheit = (int) s_cmd.fahrenheit;
}
...
static esp_err_t on_attribute_update(attribute::callback_type_t type, uint16_t endpoint_id, uint32_t cluster_id,
  ...
  if (type != attribute::POST_UPDATE || s_from_bus)
    return ESP_OK;
  ...
  if (endpoint_id == s_ep_id && cluster_id == Identify::Id && attribute_id == Identify::Attributes::IdentifyTime::Id &&
      val->val.u16 == 77) {
  ...
  if (endpoint_id == s_ep_sleep && cluster_id == ModeSelect::Id &&
    ...
    if (!(st.valid && val->val.u8 == (uint8_t) (st.sleep_raw / 2)))
      apply_sleep(val->val.u8);
    return ESP_OK;
  }

  // ---- Room A/C endpoint (ep1) ---------------------------------------------------------
  if (endpoint_id != s_ep_id)
    return ESP_OK;
  ...
  if (cluster_id == OnOff::Id && attribute_id == OnOff::Attributes::OnOff::Id) {
    ...
    if (attribute_id == Thermostat::Attributes::SystemMode::Id) {
      ...
      if (st.valid && whole_c == st.setpoint_c)
        return ESP_OK;  // echo guard (pre-clamp)
    ...
    if (attribute_id == ThermostatUserInterfaceConfiguration::Attributes::TemperatureDisplayMode::Id) {
      ...
      if (!st.valid)
        return ESP_OK;
      if (st.temp_unit_f == want_f)
        return ESP_OK;  // echo guard
      ...
      uint8_t f[HISENSE_CMD_FRAME_LEN + 2];
      size_t n = hisense_build_command_override(&s_cmd, f, sizeof(f), 23, want_f ? 0x03 : 0x01);
      if (n > 0 && hisense_send_frame(f, n)) {
        arm_sync_hold();
        ESP_LOGI(TAG, "display unit -> %s (setpoint re-encoded %d C -> %d)", want_f ? "F" : "C", (int) keep_c,
                 (int) s_cmd.setpoint);
      } else {
        ...
        ESP_LOGE(TAG, "display unit change REJECTED (n=%u) -- shadow restored", (unsigned) n);
      }
    }
...
static void set_temp_measured(uint16_t ep, chip::app::DataModel::Nullable<int16_t> v) {
  ...
  auto *iface = esp_matter::data_model::provider::get_instance().registry().Get(
      ConcreteClusterPath(ep, Clusters::TemperatureMeasurement::Id));
  if (!iface)
    return;
...
static void set_bool_state(uint16_t ep, bool v) {
  using namespace chip::app;
  auto *iface = esp_matter::data_model::provider::get_instance().registry().Get(
      ConcreteClusterPath(ep, Clusters::BooleanState::Id));
  if (!iface)
    return;
...
static void on_status(const HisenseState *st) {
  ...
  if (recommission_window_is_open() && s_status.valid && esp_timer_get_time() > s_recommission_grace_us) {
    ...
    const bool user_touched = st->power_on != s_status.power_on || st->mode != s_status.mode ||
  ...
  if (chip::System::SystemClock().GetMonotonicTimestamp() >= s_sync_hold_until) {
    ...
    if (hisense_shadow_setpoint_from_status(st->setpoint_c, st->temp_unit_f, &shadow_sp)) {
      ...
      ESP_LOGW(TAG,
               "status setpoint %d C (%s) out of range -- keeping shadow at %d "
               "(copying it would drop every later command)",
               (int) st->setpoint_c, st->temp_unit_f ? "F panel" : "C panel", (int) s_cmd.setpoint);
    }
    HisenseFanSpeed sf = hisense_fan_raw_to_cmd(st->fan_raw);
    if (sf != HISENSE_FAN_NOCHANGE)
      s_cmd.fan = sf;  // keep previous fan on an unknown raw (#59)
    ...
    s_cmd.feature = st->eco_on ? HISENSE_FEATURE_ECO : st->turbo_on ? HISENSE_FEATURE_TURBO : HISENSE_FEATURE_NONE;
  }
 ...
 *
 * HW-validated on node 35 (#72): endpoint::disable removes the ep from the Descriptor PartsList and
 * matter-server picks it up. But a RUNTIME hide leaves the HA entity lingering (unavailable) until an
 * integration reload -- so the retroactive path alone orphans an entity for a unit commissioned before
 * the gate applies. #102 (persist-and-gate-at-boot, gate_apply_persisted below) fixes that by applying
 * the LAST-SEEN gate at boot before the interview. Still bench-pending: the cool_heat FeatureMap
 * runtime change's HA mode re-derivation is unproven. Compare the exposed PartsList/FeatureMap against
 * 72-baseline.json. */
static void apply_capability_gates(const HisenseFeatures *f) {
  ...
  if (eco == g_eco && quiet == g_quiet && display == g_display && fm == g_fm)
    return;  // common case: no-op

  // PRECONDITION: caller must NOT already hold the CHIP stack lock (both call sites -- the boot
  // gate and the bus-task on_features -- are lock-free). A future reentrant caller would deadlock.
  lock::ScopedChipStackLock lk(portMAX_DELAY);
...
/* #102 persist-and-gate-at-boot: cache the last-seen features (masked to the GATE bits) in NVS so
 * the gate is applied at boot before the interview. A feature-limited unit then never advertises the
 * absent endpoints in its interview snapshot (the retroactive path corrected AFTER the interview,
 * orphaning an unavailable HA entity -- see the #72 node-35 validation).
 * PRECONDITION: capabilities are static per physical unit, so persisted == live. On a VIRGIN flash
 * nothing is stored -> gate_apply_persisted no-ops -> permissive -> on_features learns + persists ->
 * clean from boot 2 (a two-boot scheme, for a never-observed unit). A factory RESET does not
 * necessarily wipe this custom namespace, so a reset of the SAME unit keeps the (correct) gate and
 * boot 1 is already clean. The only mis-gate is flash reused across DIFFERENT units (bench) -- the
 * live path then corrects it (with the runtime-hide re-interview caveat). Flash-wear-guarded; the
 * gate mask means a non-gating bit flip on the ~60s poll neither rewrites flash nor moves the gate.
 * TODO(bench): confirm nvs_flash_erase scope on factory reset; add nvs_erase_key if the namespace
 * survives and a guaranteed permissive-after-reset is wanted. */
#define GATE_NVS_NS "hisense_gate"
#define GATE_NVS_KEY "feat32"
static void gate_persist_features(uint32_t bm) {
  bm &= HISENSE_FEAT1_GATE_MASK;  // persist only the bits the gate consumes (flash-wear + stability)
  nvs_handle_t h;
  if (nvs_open(GATE_NVS_NS, NVS_READWRITE, &h) != ESP_OK)
    return;
  uint32_t cur = 0;
  if (nvs_get_u32(h, GATE_NVS_KEY, &cur) == ESP_OK && cur == bm) {
    nvs_close(h);
    return;
  }  // unchanged: save flash
  if (nvs_set_u32(h, GATE_NVS_KEY, bm) == ESP_OK)
    nvs_commit(h);
  nvs_close(h);
}
static void gate_apply_persisted(void) {
  nvs_handle_t h;
  if (nvs_open(GATE_NVS_NS, NVS_READONLY, &h) != ESP_OK)
    return;  // first boot: namespace absent -> permissive
  uint32_t bm = 0;
  esp_err_t e = nvs_get_u32(h, GATE_NVS_KEY, &bm);
  nvs_close(h);
  if (e != ESP_OK || !((bm >> HISENSE_FEAT1_VALID) & 1))
    return;  // nothing valid cached -> stay permissive
  HisenseFeatures f;
  hisense_features_from_bitmap32(bm, &f);
...
static void on_features(const HisenseFeatures *f) {
  ESP_LOGI(TAG, "A/C features (0x66/40): ai=%d display=%d swing8=%d eco=%d mute=%d purify=%d", f->ai, f->power_display,
           f->swing_dir_8, f->power_save, f->fan_mute, f->purify);
  apply_capability_gates(f);
  if (f && f->valid)
    gate_persist_features(hisense_features_to_bitmap32(f));  // #102
}
 ...
 public:
  void OnFabricCommitted(const chip::FabricTable &, chip::FabricIndex newIndex) override {
    ...
    for (uint8_t i = 0; i < s_old_fabric_count; i++)
      if (s_old_fabrics[i] == newIndex)
        return;  // not a newly-added fabric
    ESP_LOGI(TAG, "recommission: new fabric %u joined -> deleting %u old fabric(s)", newIndex, s_old_fabric_count);
...
static void recommission_finish(bool paired, const char *why) {
  ...
  ESP_LOGI(TAG, "recommission: %s (%s) -> window closed, BLE advert off", paired ? "paired" : "reverted", why);
}

// Window expired with no new pairing -> keep the old fabric, tell the A/C to exit "77".
static void recommission_timeout(chip::System::Layer *, void *) { recommission_finish(false, "window expired"); }
...
static bool recommission_window_is_open(void) { return s_recommission_pending; }

static void recommission_user_cancel(intptr_t) { recommission_finish(false, "A/C left 77"); }
...
static void recommission_open_window(intptr_t) {
  ...
  CHIP_ERROR err = chip::Server::GetInstance().GetCommissioningWindowManager().OpenBasicCommissioningWindow(
      chip::System::Clock::Seconds32(kRecommissionWindowSec));
  ...
  ESP_LOGI(TAG, "recommission: window open %us, snapshot %u old fabric(s)", (unsigned) kRecommissionWindowSec,
           s_old_fabric_count);
}
...
static void on_link_frame(const uint8_t *f, uint8_t n) {
  ...
  ESP_LOGW(TAG, "LINK#%u len=%u b17=0x%02x | %s", (unsigned) ++seq, (unsigned) n, n > 17 ? f[17] : 0, hex);
}
  ...
  void HandleIdleStateExit() override {
    ...
    if (!m_tx_saved) {     // capture the live ceiling ONCE, on the first exit-from-idle
      ...
      if (m_tx_saved) {
        ...
        ESP_LOGW(TAG, "Matter OTA: Wi-Fi TX power %d -> %d (quarter-dBm) to cut the current peak", (int) m_saved_tx,
                 HISENSE_OTA_TX_POWER_QDBM);
      }
    }
...
static void apply_wifi_power_save(void) {
  const bool ble_window = chip::Server::GetInstance().GetCommissioningWindowManager().IsCommissioningWindowOpen() ||
  ...
  if (esp_wifi_get_ps(&have) == ESP_OK && have == want)
    return;  // already there, stay quiet
  ...
  ESP_LOGW(TAG, "Wi-Fi power save -> %s%s", want == WIFI_PS_NONE ? "NONE" : "MIN_MODEM",
...
static void wifi_connectivity_log_handler(const chip::DeviceLayer::ChipDeviceEvent *event, intptr_t) {
  if (event->Type != chip::DeviceLayer::DeviceEventType::kWiFiConnectivityChange)
    return;
...
static void https_ota_task(void *arg) {
  ...
  if (tx_saved) {
    esp_wifi_set_max_tx_power(HISENSE_OTA_TX_POWER_QDBM);
    ESP_LOGW(TAG, "HTTPS-OTA: Wi-Fi TX power %d -> %d (quarter-dBm) to cut the current peak", (int) saved_tx,
             HISENSE_OTA_TX_POWER_QDBM);
  }
  ...
  if (err != ESP_OK || h == NULL) {
    ...
    if (tx_saved)
      esp_wifi_set_max_tx_power(saved_tx);
    ota_activity_end();
    vTaskDelete(NULL);
    return;
  }
  ...
  if (err == ESP_OK && complete) {
    ...
    ESP_LOGE(TAG, "HTTPS-OTA: incomplete (err=%s complete=%d) -- aborting, NOT booting it", esp_err_to_name(err),
             (int) complete);
...
static void breakglass_task(void *arg) {
  ...
  (void) arg;
  if (srv < 0) {
    ESP_LOGE(TAG, "break-glass: socket failed");
    vTaskDelete(NULL);
    return;
  }
  ...
  for (;;) {
    ...
    if (breakglass_token_ok(buf, len)) {
      static const char kOk[] = "ok: fetching; device reboots into the idle slot.\r\n"
...
static void breakglass_start(void) { xTaskCreate(breakglass_task, "breakglass", 4096, NULL, 5, NULL); }
...
class AppDeviceInfoProvider : public chip::DeviceLayer::DeviceInfoProvider {
 public:
  static AppDeviceInfoProvider &Instance() {
    static AppDeviceInfoProvider i;
    return i;
  }
  ...
  UserLabelIterator *IterateUserLabel(chip::EndpointId ep) override { return chip::Platform::New<Iter>(mStore[ep]); }
 ...
 protected:
  ...
  CHIP_ERROR GetUserLabelLength(chip::EndpointId ep, size_t &val) override {
    auto it = mStore.find(ep);
    val = (it == mStore.end()) ? 0 : it->second.size();
    return CHIP_NO_ERROR;
  }
  CHIP_ERROR SetUserLabelLength(chip::EndpointId ep, size_t val) override {
    mStore[ep].resize(val);
    return CHIP_NO_ERROR;
  }
...
static const char *reset_reason_str(esp_reset_reason_t r) {
  switch (r) {
    ...
    case ESP_RST_EXT:
      return "external pin";
    case ESP_RST_SW:
      return "software restart (expected after our OTA)";
    case ESP_RST_PANIC:
    ...
    case ESP_RST_USB:
      return "USB peripheral reset (reflash / host reset -- benign)";
    case ESP_RST_JTAG:
      return "JTAG reset (debugger -- benign)";
    case ESP_RST_EFUSE:
      return "efuse error";
    case ESP_RST_PWR_GLITCH:
      return "POWER GLITCH (supply integrity -- investigate)";
...
extern "C" void diag_get_boot_reason(int *code, const char **text) {
  if (code)
    *code = (int) s_boot_reason;
  if (text)
    *text = reset_reason_str(s_boot_reason);
}
...
static bool network_is_live(void) {
  ...
  esp_ip6_addr_t ll = {};
  if (esp_netif_get_ip6_linklocal(sta, &ll) != ESP_OK)
    return false;  // Matter is IPv6-only
...
static void health_watchdog_task(void *arg) {
  ...
  for (;;) {
    ...
    ESP_LOGI(TAG, "heap: free=%u min_free=%u (bytes)", (unsigned) esp_get_free_heap_size(),
    ...
    if (s_ota_active && (now - s_ota_started_us) < HISENSE_OTA_GATE_MAX_US) {
      ESP_LOGW(TAG, "net watchdog: no live network for %llds, OTA in flight -- holding off", (long long) dead_s);
      continue;
    }

    if ((now - last_good_us) < HISENSE_NET_DEAD_REBOOT_US) {
      ESP_LOGW(TAG, "net watchdog: no live network for %llds (reboot at %llds)", (long long) dead_s,
               (long long) (HISENSE_NET_DEAD_REBOOT_US / 1000000));
      continue;
    }

    ESP_LOGE(TAG, "net watchdog: no live network for %llds -- REBOOTING to recover the stack", (long long) dead_s);
...
extern "C" void app_main() {
  ...
  // node). The window being open is fine: a controller only reads the ENDPOINT interview after the
  // PASE+CASE handshake (seconds later), whereas this runs sub-ms after start() -- so a feature-
  // limited unit's interview snapshot still excludes the absent endpoints. Reads from NVS (no bus
  // wait), so far earlier than the retroactive on_features path. Nothing persisted -> no-op
  // (permissive); on_features re-derives + re-persists. Runs before hisense_init() spawns the bus
  // task, so it and on_features never touch the apply_capability_gates static guards concurrently.
  ...
  if (chip::Server::GetInstance().GetFabricTable().FabricCount() == 0) {
    chip::DeviceLayer::PlatformMgr().LockChipStack();
    CHIP_ERROR werr = chip::Server::GetInstance().GetCommissioningWindowManager().OpenBasicCommissioningWindow(
        chip::System::Clock::Seconds32(kUncommissionedWindowSec));
    chip::DeviceLayer::PlatformMgr().UnlockChipStack();
    if (werr == CHIP_NO_ERROR) {
      ESP_LOGW(TAG, "no fabrics: commissioning window held open for %d s (BLE + IP)", (int) kUncommissionedWindowSec);
    } else {
      ESP_LOGE(TAG, "no fabrics but OpenBasicCommissioningWindow failed: %" CHIP_ERROR_FORMAT, werr.Format());
    }
  }
