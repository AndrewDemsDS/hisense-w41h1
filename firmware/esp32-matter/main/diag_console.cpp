...
static const char *mode_name(HisenseMode m) {
  switch (m) {
    case HISENSE_MODE_FAN:
      return "fan";
    case HISENSE_MODE_HEAT:
      return "heat";
    case HISENSE_MODE_COOL:
      return "cool";
...
static void print_state(FILE *out, const HisenseState *st) {
  fprintf(out,
          ...
          st->power_on, mode_name(st->mode), st->setpoint_c, st->indoor_temp_c, st->outdoor_temp_c, st->coil_temp_c,
          st->fan_raw, st->compressor_freq, st->eco_on, st->turbo_on, st->mute_on, st->sleep_on, st->sleep_raw,
          st->vswing_on, st->hswing_on, st->heat_relay_on, (unsigned) st->current_raw, (unsigned) st->voltage_raw,
          st->temp_unit_f ? "F" : "C", (unsigned) hisense_get_last_link_req());
}

extern "C" void diag_on_status(const HisenseState *st) {
  if (!st || !st->valid || !s_mtx)
    return;
  LOCK();
  s_snap = *st;
  s_have = true;
...
static int cmd_token(int, char **) {
  ...
  printf("  device-type  inner [3]/[4] : %02x %02x  [%s]   <- USED (stock 0x9b6f2194)\r\n", dhi, dlo,
         dseen ? "learned from A/C" : "DEFAULT 01 01 — no 0x0A reply parsed");
  if (eseen)
...
static int cmd_features(int, char **) {
  ...
  printf("  cool_heat=%d ai=%d infinite_fan=%d power_save/eco=%d fan_mute/quiet=%d\r\n", ft.cool_heat, ft.ai,
         ft.infinite_fan, ft.power_save, ft.fan_mute);
  printf("  swing_dir_8=%d swing_follow=%d humidity=%d power_display=%u demand_resp=%u\r\n", ft.swing_dir_8,
         ft.swing_follow, ft.humidity, (unsigned) ft.power_display, (unsigned) ft.demand_resp);
...
static int cmd_poll(int, char **) {
  ...
  if (!h) {
    printf("no status decoded yet (is the A/C bus connected + powered?)\r\n");
    return 0;
  }
...
static int cmd_decode(int argc, char **argv) {
  ...
  if (hisense_parse_status(f, (size_t) n, &st) && st.valid) {
    ...
    printf("producttype (%dB): cool_heat=%d ai=%d power_save=%d fan_mute=%d swing8=%d purify=%d\r\n", n, ft.cool_heat,
           ft.ai, ft.power_save, ft.fan_mute, ft.swing_dir_8, ft.purify);
  else
    printf("not a valid status/producttype frame (class byte[13]=0x%02x)\r\n", n > 13 ? f[13] : 0);
  return 0;
}
...
static int cmd_tx(int argc, char **argv) {
  ...
  switch (diag_tx_override((int) off, (uint8_t) val)) {
    case -1:
      printf("rejected: offset %ld is outside the payload [%u,%u)\r\n", off, (unsigned) HISENSE_CMD_HEADER_LEN,
             (unsigned) HISENSE_CMD_CHK_OFFSET);
      return 1;
    case -2:
...
static int cmd_faults(int, char **) {
  ...
  printf("raw: [%d]=0x%02x [%d]=0x%02x [%d]=0x%02x [%d]=0x%02x  ->  %s\r\n", HISENSE_FAULT_BYTE_INDOOR, f.raw_indoor,
         HISENSE_FAULT_BYTE_MODULE, f.raw_module, HISENSE_FAULT_BYTE_OUTDOOR, f.raw_outdoor, HISENSE_FAULT_BYTE_PROTECT,
         f.raw_protect,
  ...
  struct {
    ...
    const char *n;
      ...
      {f.in_temp, "indoor temp sensor"},
      {f.in_coil_temp, "indoor coil sensor"},
      {f.in_humidity, "indoor humidity"},
      {f.water_full, "condensate tray full"},
      {f.in_fan_motor, "indoor fan motor / up-machine"},
      {f.grille, "grille / dw-machine"},
      {f.in_vzero, "zero-cross detect"},
      {f.in_com, "indoor<->outdoor comms"},
      {f.in_display, "display board"},
      {f.in_keys, "keypad"},
      {f.in_wifi, "wifi module"},
      {f.in_ele, "indoor electrical"},
      {f.in_eeprom, "indoor eeprom"},
      {f.out_eeprom, "outdoor eeprom"},
      {f.out_coil_temp, "outdoor coil sensor"},
      {f.out_gas_temp, "outdoor gas sensor"},
      {f.out_temp, "outdoor temp sensor"},
  ...
  uint8_t known[4] = {0xFF, 0xF8, 0x78, (uint8_t) (0x10 | HISENSE_FAULT_NONFAULT_PROTECT)};
  uint8_t rawb[4] = {f.raw_indoor, f.raw_module, f.raw_outdoor, f.raw_protect};
  int off[4] = {HISENSE_FAULT_BYTE_INDOOR, HISENSE_FAULT_BYTE_MODULE, HISENSE_FAULT_BYTE_OUTDOOR,
                HISENSE_FAULT_BYTE_PROTECT};
  for (int i = 0; i < 4; i++) {
    uint8_t unknown = (uint8_t) (rawb[i] & ~known[i]);
    if (unknown)
      printf("  UNNAMED fault bits 0x%02x in byte %d\r\n", unknown, off[i]);
  }
  return 0;
}
...
static int cmd_link(int, char **) {
  ...
  if (!n) {
    printf("no 0x1E LINK reply captured yet\r\n");
    return 0;
  }
...
static int cmd_raw(int, char **) {
  ...
  if (!n) {
    printf("no status frame captured yet\r\n");
    return 0;
  }
  ...
  printf("  fault bytes: [%d]=0x%02x [%d]=0x%02x [%d]=0x%02x [%d]=0x%02x\r\n", HISENSE_FAULT_BYTE_INDOOR,
         n > HISENSE_FAULT_BYTE_INDOOR ? f[HISENSE_FAULT_BYTE_INDOOR] : 0, HISENSE_FAULT_BYTE_MODULE,
         n > HISENSE_FAULT_BYTE_MODULE ? f[HISENSE_FAULT_BYTE_MODULE] : 0, HISENSE_FAULT_BYTE_OUTDOOR,
         n > HISENSE_FAULT_BYTE_OUTDOOR ? f[HISENSE_FAULT_BYTE_OUTDOOR] : 0, HISENSE_FAULT_BYTE_PROTECT,
         n > HISENSE_FAULT_BYTE_PROTECT ? f[HISENSE_FAULT_BYTE_PROTECT] : 0);
...
static int cmd_selftest(int, char **) {
  ...
  if (!(n && f[16] == 0x0B && f[18] == 0x50 && f[19] == 0x2D && f[33] == 0x30))
    fails++;
  uint8_t hi = 0, lo = 0;
  ...
  if (!(hisense_devtype_from_reply(reply, 20, &hi, &lo) && hi == 0xAB && lo == 0xCD))
    fails++;
  reply[13] = 0x66;  // a status reply must NOT supply a device type
  if (hisense_devtype_from_reply(reply, 20, &hi, &lo))
    fails++;
...
static void handle_client(int fd) {
  ...
  for (;;) {
    ...
    if (e == ESP_ERR_NOT_FOUND)
      fprintf(sf, "unknown command (try 'help')\r\n");
    fprintf(sf, "esp32-recon> ");
  }
  LOCK();
  if (s_watch == sf) {
...
static void tcp_task(void *) {
  ...
  int off = 0;
  setsockopt(ls, IPPROTO_IPV6, IPV6_V6ONLY, &off, sizeof(off));  // best-effort dual-stack
  struct sockaddr_in6 a = {};
  a.sin6_family = AF_INET6;
  a.sin6_port = htons(DIAG_TCP_PORT);
  a.sin6_addr = in6addr_any;
  if (bind(ls, (struct sockaddr *) &a, sizeof(a)) < 0 || listen(ls, 1) < 0) {
    ESP_LOGE(TAG, "bind/listen :%d failed", DIAG_TCP_PORT);
    close(ls);
    vTaskDelete(NULL);
    return;
  }
...
extern "C" void diag_console_start(void) {
  ...
  if (esp_console_init(&cc) == ESP_OK) {
    ...
    const esp_console_cmd_t cmds[] = {
        ...
        {"bootreason", "#12: why the module last rebooted (brownout / panic / OTA)", NULL, cmd_bootreason, NULL, NULL},
        ...
        {"tx", "#52 bench probe: tx <offset> <value> — current frame, one byte overridden", NULL, cmd_tx, NULL, NULL},
        {"busstats", "raw HAL tx/rx byte counters — is the silence ours or theirs?", NULL, cmd_busstats, NULL, NULL},
        {"pintest", "electrical check of the DE/EN pin (drive + read back) — no meter needed", NULL, cmd_pintest, NULL,
         NULL},
    };
  ...
  ESP_LOGI(TAG, "diag console on :%d — reach it at the node's Matter IPv6 (nc <ipv6> %d)", DIAG_TCP_PORT,
           DIAG_TCP_PORT);
}
