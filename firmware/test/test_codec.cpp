...
//
// Build/run:  ./run_tests.sh   (or: g++ -I. -I../src/rs485-driver test_codec.cpp ../src/rs485-driver/hisense_rs485.cpp
// -o test_codec && ./test_codec)
...
static size_t make_status(uint8_t *out, bool power, HisenseMode mode, int8_t setp_c, int8_t indoor_c, uint8_t fan_raw,
                          uint8_t flags1, uint8_t flags2, uint8_t sleep_raw, uint8_t comp_hz, int8_t outdoor_c) {
...
int main() {
  ...
  {
    HisenseCommand c = {HISENSE_MODE_AUTO,
                        24,
                        false,
                        HISENSE_FAN_AUTO,
                        HISENSE_SWING_OFF,
    ...
    CHECK(f[18] == 0x90, "AUTO byte18=0x%02X exp 0x90", f[18]);
  }
  {
    HisenseCommand c = {HISENSE_MODE_COOL,
                        24,
                        false,
                        HISENSE_FAN_AUTO,
                        HISENSE_SWING_OFF,
                        ...
                        HISENSE_DISPLAY_NOCHANGE};
    hisense_build_command(&c, f, sizeof(f));
    CHECK(f[18] == 0x50, "COOL byte18=0x%02X exp 0x50", f[18]);
  }
  {
    HisenseCommand c = {HISENSE_MODE_HEAT,
                        24,
                        false,
                        HISENSE_FAN_AUTO,
                        HISENSE_SWING_OFF,
                        ...
                        HISENSE_DISPLAY_NOCHANGE};
    hisense_build_command(&c, f, sizeof(f));
    CHECK(f[18] == 0x30, "HEAT byte18=0x%02X exp 0x30", f[18]);
  }
  {
    HisenseCommand c = {HISENSE_MODE_DRY,
                        24,
                        false,
                        HISENSE_FAN_AUTO,
                        HISENSE_SWING_OFF,
                        ...
                        HISENSE_DISPLAY_NOCHANGE};
    hisense_build_command(&c, f, sizeof(f));
    CHECK(f[18] == 0x70, "DRY byte18=0x%02X exp 0x70", f[18]);
  }
  {
    HisenseCommand c = {HISENSE_MODE_FAN,
                        24,
                        false,
                        HISENSE_FAN_AUTO,
                        HISENSE_SWING_OFF,
                        ...
                        HISENSE_DISPLAY_NOCHANGE};
    hisense_build_command(&c, f, sizeof(f));
    CHECK(f[18] == 0x10, "FANmode byte18=0x%02X exp 0x10", f[18]);
  }
  // six fan speeds
  struct {
    HisenseFanSpeed s;
    uint8_t b;
    const char *n;
  } fans[] = {{HISENSE_FAN_AUTO, 0x01, "auto"}, {HISENSE_FAN_QUIET, 0x03, "quiet"},
              {HISENSE_FAN_LOW, 0x0B, "low"},   {HISENSE_FAN_MED_LOW, 0x0D, "med-low"},
              {HISENSE_FAN_MID, 0x0F, "mid"},   {HISENSE_FAN_MED_HIGH, 0x11, "med-high"},
              {HISENSE_FAN_HIGH, 0x13, "high"}};
  for (auto &x : fans) {
    HisenseCommand c = {
        HISENSE_MODE_COOL,       24, false, x.s, HISENSE_SWING_OFF, HISENSE_SWING_OFF, HISENSE_FEATURE_NONE,
        HISENSE_DISPLAY_NOCHANGE};
    hisense_build_command(&c, f, sizeof(f));
    CHECK(f[16] == x.b, "fan %s byte16=0x%02X exp 0x%02X", x.n, f[16], x.b);
  }
  // temp 22C -> 2n+1 = 0x2D
  {
    HisenseCommand c = {HISENSE_MODE_COOL,
                        ...
                        HISENSE_DISPLAY_NOCHANGE};
    hisense_build_command(&c, f, sizeof(f));
    CHECK(f[19] == 0x2D, "temp22 byte19=0x%02X exp 0x2D", f[19]);
  }
  // eco / turbo
  {
    HisenseCommand c = {
        HISENSE_MODE_COOL,       24, false, HISENSE_FAN_AUTO, HISENSE_SWING_OFF, HISENSE_SWING_OFF, HISENSE_FEATURE_ECO,
        HISENSE_DISPLAY_NOCHANGE};
    hisense_build_command(&c, f, sizeof(f));
    CHECK(f[33] == 0x30, "eco byte33=0x%02X exp 0x30", f[33]);
  }
  {
    HisenseCommand c = {HISENSE_MODE_COOL,
                        24,
                        ...
                        HISENSE_SWING_OFF,
                        HISENSE_SWING_OFF,
                        HISENSE_FEATURE_TURBO,
                        HISENSE_DISPLAY_NOCHANGE};
    hisense_build_command(&c, f, sizeof(f));
    CHECK(f[33] == 0x0C, "turbo byte33=0x%02X exp 0x0C", f[33]);
  }
  // vertical swing
  {
    HisenseCommand c = {HISENSE_MODE_COOL,
                        ...
                        HISENSE_DISPLAY_NOCHANGE};
    hisense_build_command(&c, f, sizeof(f));
    CHECK(f[32] == 0xC0, "vswing byte32=0x%02X exp 0xC0", f[32]);
  }
  // every command frame must carry a valid 2-byte checksum + F4 FB end
  {
    ...
    uint16_t ck = besum(f, (int) n - 4);
    CHECK(((f[n - 4] << 8) | f[n - 3]) == ck, "cmd checksum mismatch");
  ...
  {
    int found = 0;
    HisenseMode modes[5] = {HISENSE_MODE_COOL, HISENSE_MODE_HEAT, HISENSE_MODE_DRY, HISENSE_MODE_FAN,
                            HISENSE_MODE_AUTO};
      ...
      for (int mi = 0; mi < 5; mi++)
        for (int fi = 0; fi < 3; fi++)
          for (int qi = 0; qi < 3; qi++) {
            HisenseCommand c = {modes[mi],         (int8_t) s,        false,     fans[fi],
                                HISENSE_SWING_OFF, HISENSE_SWING_OFF, feats[qi], HISENSE_DISPLAY_NOCHANGE};
            uint8_t o[64];
            size_t n = hisense_build_command(&c, o, sizeof(o));
            if (n > HISENSE_CMD_FRAME_LEN) {
              found++;
              bool dbl = false;
              for (size_t i = HISENSE_CMD_CHK_OFFSET; i + 2 < n; i++)
                if (o[i] == 0xF4 && o[i + 1] == 0xF4)
                  dbl = true;
  ...
  {
    uint8_t o[64];
    size_t n = hisense_build_power_frame(true, o, sizeof(o));
    CHECK(o[0] == 0xF4 && o[1] == 0xF5 && o[n - 2] == 0xF4 && o[n - 1] == 0xFB, "power-on framing");
    uint16_t ck = besum(o, (int) n - 4);
    CHECK(((o[n - 4] << 8) | o[n - 3]) == ck, "power-on checksum valid");
  ...
  {
    uint8_t o[64];
    size_t n = hisense_build_mute_frame(true, o, sizeof(o));
    CHECK(o[35] == 0x30, "mute on byte35=0x%02X exp 0x30", o[35]);
    uint16_t ck = besum(o, (int) n - 4);
    CHECK(((o[n - 4] << 8) | o[n - 3]) == ck, "mute frame checksum");
  }
  {
    uint8_t o[64];
    hisense_build_mute_frame(false, o, sizeof(o));
    CHECK(o[35] == 0x10, "mute off byte35=0x%02X exp 0x10", o[35]);
  }
  {
    uint8_t o[64];
    struct {
      uint8_t p, b;
    } sp[] = {{0, 0x01}, {1, 0x03}, {2, 0x05}, {3, 0x07}, {4, 0x09}};
    for (auto &x : sp) {
      hisense_build_sleep_frame(x.p, o, sizeof(o));
      CHECK(o[17] == x.b, "sleep %d byte17=0x%02X exp 0x%02X", x.p, o[17], x.b);
    }
  }
  ...
  hisense_parse_status(s, 160, &st);
  CHECK(st.vswing_on, "vswing");
  CHECK(st.eco_on, "eco");
  CHECK(st.turbo_on, "turbo");
  CHECK(st.mute_on, "mute");
  CHECK(st.fan_raw == 0x12, "fan_raw hi");
  CHECK(st.compressor_freq == 55, "comp %d", st.compressor_freq);
  ...
  {
    uint16_t ck = besum(s, 156);
    s[156] = (uint8_t) (ck >> 8);
    s[157] = (uint8_t) (ck & 0xFF);
  }
  ...
  make_status(s, true, HISENSE_MODE_COOL, 22, 21, 0x01, 0, 0, 0, 0, 32);
  s[4] = 0xA5;  // corrupt LEN byte
  uint16_t ck = besum(s, 156);
  s[156] = ck >> 8;
  s[157] = ck & 0xFF;  // re-checksum so only length-formula check fails
  ...
  {
    uint16_t ck = besum(s, 156);
    CHECK(((s[156] << 8) | s[157]) == ck, "golden frame checksum is self-consistent");
  }
  {
    uint8_t c[160];
    memcpy(c, s, 160);
    c[100] ^= 0xFF;  // corrupt a body byte, do NOT re-checksum
    CHECK(!hisense_status_checksum_ok(c, 160), "checksum_ok rejects a corrupted body byte");
  }
  {
    uint8_t c[160];
    memcpy(c, s, 160);
    c[156] ^= 0xFF;  // corrupt the stored checksum hi byte
    CHECK(!hisense_status_checksum_ok(c, 160), "checksum_ok rejects a corrupted checksum byte");
  }
  {
    uint8_t c[160];
    memcpy(c, s, 160);
    c[158] = 0x00;  // clobber the F4 FB end tag
  ...
  printf("[recommission re-entry lockout #69]\n");
  {
    ...
    CHECK(hisense_smartcfg_lockout_mask(HISENSE_LINK_REQ_RECONFIG, 50000, UNTIL) == HISENSE_LINK_REQ_RECONFIG,
          "lockout: 0x08 passes through untouched");
    CHECK(hisense_smartcfg_lockout_mask(HISENSE_LINK_REQ_RECOMMISSION, 50000, UNTIL) == HISENSE_LINK_REQ_RECONFIG,
          "lockout: mixed 0x28 -> 0x08 only");
    // Deadline itself is expired (signed delta == 0 is not "in the future").
    CHECK(hisense_smartcfg_lockout_mask(HISENSE_LINK_REQ_SMARTCFG, UNTIL, UNTIL) == HISENSE_LINK_REQ_SMARTCFG,
          "at the deadline the lockout has expired");
    // After the lockout: the same pulse fires normally again (deliberate re-entry works).
    CHECK(hisense_smartcfg_lockout_mask(HISENSE_LINK_REQ_SMARTCFG, 110000, UNTIL) == HISENSE_LINK_REQ_SMARTCFG,
          "after lockout: 0x20 passes through");
    // No lockout armed (boot state): nothing is ever stripped.
    CHECK(hisense_smartcfg_lockout_mask(HISENSE_LINK_REQ_SMARTCFG, 0, 0) == HISENSE_LINK_REQ_SMARTCFG,
          "until==0 (never armed) passes 0x20");
    // Tick wrap: deadline just past the wrap is still in the future for a `now` near it.
    CHECK(hisense_smartcfg_lockout_mask(HISENSE_LINK_REQ_SMARTCFG, 0xFFFFFFF0u, 50) == 0, "lockout survives tick wrap");
  ...
  printf("[0x66/40 feature-flags parse + request]\n");
  {
    ...
    ff[14] = 0x00;
    CHECK(!hisense_parse_features(ff, 64, &fe), "reject non-0x40 subtype");
    ff[14] = 0x40;
    CHECK(!hisense_parse_features(ff, 30, &fe), "reject too-short frame");
  }
  ...
  printf("[#49 devtype from reply]\n");
  {
    ...
    reply[16] = 0x36;
    reply[17] = 0x00;  // another model's type carried through
    CHECK(hisense_devtype_from_reply(reply, 20, &hi, &lo) && hi == 0x36 && lo == 0x00, "arbitrary devtype %02X %02X",
          hi, lo);
  ...
  printf("[#49 link token stamp]\n");
  {
    ...
    {
      size_t n =
          hisense_stamp_link_token(HISENSE_STATUS_REQUEST, HISENSE_STATUS_REQUEST_LEN, 0xAB, 0xCD, out, sizeof(out));
    ...
    {
      size_t n =
          hisense_stamp_link_token(HISENSE_STATUS_REQUEST, HISENSE_STATUS_REQUEST_LEN, 0x42, 0x01, out, sizeof(out));
    ...
    // Malformed / hostile envelopes are refused (caller then sends verbatim).
    {
      ...
      CHECK(hisense_stamp_link_token(HISENSE_STATUS_REQUEST, HISENSE_STATUS_REQUEST_LEN, 1, 1, out, 4) == 0,
            "reject too-small out_cap");
  ...
  // ---- C/F unit bit (#5), frame byte 26 bit 1 ------------------------------
  {
    ...
    CHECK(hisense_parse_status(s2, 160, &st2) && st2.temp_unit_f, "byte26 bit1 set -> Fahrenheit");
    // Only bit 1 may matter: the rest of byte 26 must not leak into the flag.
    set26((uint8_t) ~0x02);
    CHECK(hisense_parse_status(s2, 160, &st2) && !st2.temp_unit_f, "every other bit of byte 26 ignored");
  }
  ...
  // made a real unit target 23 F and run at 74 Hz toward -5 C, so this is not cosmetic.
  {
    ...
    for (int c = HISENSE_SETPOINT_MIN_C; c <= HISENSE_SETPOINT_MAX_C; c++) {
      ...
      CHECK(hisense_setpoint_in_range(f, true), "%dC maps to %dF, inside the F range the builder accepts", c, f);
    }
    // And a command built in F must encode the F value, not the Celsius one.
    {
      HisenseCommand c = {HISENSE_MODE_COOL,    hisense_c_to_f(22),      true,
                          HISENSE_FAN_AUTO,     HISENSE_SWING_OFF,       HISENSE_SWING_OFF,
  ...
  // command silently re-imposed it.
  {
    ...
    CHECK(hisense_shadow_setpoint_from_status(22, false, &sp) && sp == 22, "C panel: 22 C -> shadow 22 (got %d)", sp);
    // F panel sequence: first sync stores F (22 C -> 72 F)...
    CHECK(hisense_shadow_setpoint_from_status(22, true, &sp) && sp == 72, "F panel: 22 C -> shadow 72 F (got %d)", sp);
    ...
    for (int c = HISENSE_SETPOINT_MIN_C; c <= HISENSE_SETPOINT_MAX_C; c++) {
      ...
      CHECK(hisense_shadow_setpoint_from_status((int8_t) c, false, &sp) && sp == c, "C panel: %d C accepted", c);
    }
    ...
    // equivalent 41 F.
    sp = -7;
    CHECK(!hisense_shadow_setpoint_from_status(5, false, &sp) && sp == -7, "5 C report refused, shadow untouched");
    CHECK(!hisense_shadow_setpoint_from_status(5, true, &sp) && sp == -7,
          "5 C (41 F) report on an F panel refused, shadow untouched");
    CHECK(!hisense_shadow_setpoint_from_status(40, false, &sp) && sp == -7, "40 C report refused");
  }
  ...
  // index - 8 = bit in that byte. Frame bytes 39/40/64/66.
  {
    ...
    for (auto &c : cases) {
      make_status(s, true, HISENSE_MODE_COOL, 22, 25, 0x01, 0x00, 0x00, 0x00, 40, 30);
      s[c.byte] = c.mask;
      CHECK(hisense_parse_faults(s, 160, &fl) && fl.*(c.field), "byte %d mask 0x%02X -> %s set", c.byte, c.mask,
            c.name);
    ...
    CHECK(hisense_parse_faults(s, 160, &fl) && fl.any && fl.over_temp, "byte66 bit4 still reports over-temp");
    ...
    CHECK(hisense_parse_faults(s, 160, &fl) && fl.any, "other unnamed bits in byte66 still raise any");
    ...
    CHECK(hisense_parse_faults(s, 160, &fl) && fl.any, "unnamed fault bit still raises any");
  ...
  // other half). Layout is defined in hisense_rs485.h.
  {
    ...
    HisenseFaults fh = {};
    fh.valid = true;
    CHECK(hisense_faults_to_bitmap32(&fh) == (1u << HISENSE_FAULT1_VALID), "healthy faults -> only VALID bit");
    ...
    ed.power_display = 3;
    ed.demand_resp = 2;
    uint32_t wantd = (1u << HISENSE_FEAT1_VALID) | (3u << HISENSE_FEAT1_POWER_DISPLAY_SHIFT) |
    ...
    ex.valid = true;
    ex.q_display = true;
    ex.enable_8heat = true;
    CHECK(hisense_features_to_bitmap32(&ex) == (1u << HISENSE_FEAT1_VALID), "ext fields dropped when ext_valid=false");
    ex.ext_valid = true;
    uint32_t wantx = (1u << HISENSE_FEAT1_VALID) | (1u << HISENSE_FEAT1_EXT_VALID) | (1u << HISENSE_FEAT1_Q_DISPLAY) |
                     (1u << HISENSE_FEAT1_ENABLE_8HEAT);
  ...
  // it has to agree with the builder exactly.
  {
    ...
    for (int sp = 0; sp <= 40; sp++) {
      ...
      CHECK(built == predict, "setpoint %d: builder=%d helper=%d (must agree)", sp, built, predict);
    }
    for (int sp = 0; sp <= 40; sp += 8) {
      HisenseCommand c = {HISENSE_MODE_FAN,
                          ...
                          HISENSE_FEATURE_NONE,
                          HISENSE_DISPLAY_NOCHANGE};
      CHECK(hisense_build_command(&c, o, sizeof(o)) > 0, "FAN mode builds at setpoint %d (setpoint stripped, #53)", sp);
    }
  }
  ...
  // stomp the panel. The old code sent 0x00 for OFF, which is why off never worked.
  {
    ...
    for (auto &x : cases) {
      HisenseCommand c = {HISENSE_MODE_COOL,    24, false, HISENSE_FAN_AUTO, HISENSE_SWING_OFF, HISENSE_SWING_OFF,
  ...
  // test, so a miss is a no-op instead of a surprise mode/setpoint change.
  {
    ...
    HisenseCommand c = {
        HISENSE_MODE_COOL,       22, false, HISENSE_FAN_LOW, HISENSE_SWING_OFF, HISENSE_SWING_OFF, HISENSE_FEATURE_ECO,
        HISENSE_DISPLAY_NOCHANGE};
    ...
    CHECK(hisense_build_command_override(&c, ovr, sizeof(ovr), (int) HISENSE_CMD_HEADER_LEN - 1, 0xFF) == 0,
          "reject last header byte");
    CHECK(hisense_build_command_override(&c, ovr, sizeof(ovr), (int) HISENSE_CMD_CHK_OFFSET, 0xFF) == 0,
          "reject checksum offset");
    CHECK(hisense_build_command_override(&c, ovr, sizeof(ovr), (int) HISENSE_CMD_END_OFFSET, 0xFF) == 0,
          "reject terminator offset");
    CHECK(hisense_build_command_override(&c, ovr, sizeof(ovr), -1, 0xFF) == 0, "reject negative offset");
    CHECK(hisense_build_command_override(&c, ovr, sizeof(ovr), (int) HISENSE_CMD_HEADER_LEN, 0xFF) > 0,
          "accept first payload offset");
    CHECK(hisense_build_command_override(&c, ovr, sizeof(ovr), (int) HISENSE_CMD_CHK_OFFSET - 1, 0xFF) > 0,
          "accept last payload offset");
  }
  ...
  // 22 F = -6 C and the unit drove toward it at full compressor.
  {
    ...
    HisenseCommand to_f = {
        HISENSE_MODE_COOL,       0, true, HISENSE_FAN_LOW, HISENSE_SWING_OFF, HISENSE_SWING_OFF, HISENSE_FEATURE_NONE,
        HISENSE_DISPLAY_NOCHANGE};
    ...
    HisenseCommand to_c = {
        HISENSE_MODE_COOL,       22, false, HISENSE_FAN_LOW, HISENSE_SWING_OFF, HISENSE_SWING_OFF, HISENSE_FEATURE_NONE,
        HISENSE_DISPLAY_NOCHANGE};
    ...
    CHECK(hisense_build_command(&bad, f, sizeof(f)) == 0, "F setpoint mislabelled as C is REJECTED, not silently sent");
