...
int main() {
  ...
  printf("[mode -> command byte18]\n");
  {
    ...
    matter_mode_to_hisense(3, &c.mode);
    CHECK(cmd_byte(c, 18) == 0x50, "Cool -> byte18 0x%02X exp 0x50", cmd_byte(c, 18));
  }
  {
    HisenseCommand c = base;
    matter_mode_to_hisense(1, &c.mode);
    CHECK(cmd_byte(c, 18) == 0x90, "Auto -> byte18 0x%02X exp 0x90", cmd_byte(c, 18));
  }
  {
    HisenseCommand c = base;
    matter_mode_to_hisense(4, &c.mode);
    CHECK(cmd_byte(c, 18) == 0x30, "Heat -> byte18 0x%02X exp 0x30", cmd_byte(c, 18));
  }
  ...
  struct {
    uint8_t sp;
    uint8_t b;
  } speeds[] = {{1, 0x03}, {2, 0x0B}, {3, 0x0D}, {4, 0x0F}, {5, 0x11}, {6, 0x13}};
  for (auto &s : speeds) {
    HisenseCommand c = base;
    c.fan = speed_to_hisense_fan(s.sp);
  ...
  struct {
    ...
    uint8_t b;
    const char *n;
  } fanmodes[] = {{1, 0x0B, "Low"}, {2, 0x0F, "Medium"}, {3, 0x13, "High"},
                  {4, 0x13, "On"},  {5, 0x01, "Auto"},   {0, 0x01, "Off"}};
  for (auto &m : fanmodes) {
    HisenseCommand c = base;
    c.fan = fanmode_to_hisense_fan(m.fm);
  ...
  {
    HisenseCommand c = base;
    c.fan = hisense_fan_raw_to_cmd(0x0A);
    CHECK(cmd_byte(c, 16) == 0x0B, "raw 0x0A synced -> byte16 0x%02X exp 0x0B", cmd_byte(c, 16));
  }
  {
    HisenseCommand c = base;
    c.fan = hisense_fan_raw_to_cmd(0x12);
    CHECK(cmd_byte(c, 16) == 0x13, "raw 0x12 synced -> byte16 0x%02X exp 0x13", cmd_byte(c, 16));
  }
  // the NOCHANGE sentinel must never reach the wire -- it packs the 0x01 filler (#59)
  {
    HisenseCommand c = base;
    c.fan = HISENSE_FAN_NOCHANGE;
    CHECK(cmd_byte(c, 16) == 0x01, "NOCHANGE fan -> byte16 0x%02X exp 0x01 (filler)", cmd_byte(c, 16));
  }

  // ---- swing: RockSetting <-> v/h swing -> wire byte32 ----
  printf("[swing rock <-> command byte32]\n");
  rock_to_swing(HISENSE_ROCK_UPDOWN, &v, &h);
  CHECK(v && !h, "rock updown -> vswing");
  rock_to_swing(HISENSE_ROCK_LEFTRIGHT, &v, &h);
  CHECK(!v && h, "rock leftright -> hswing");
  rock_to_swing(0x03, &v, &h);
  CHECK(v && h, "rock both -> v+h");
  CHECK(swing_to_rock(true, false) == HISENSE_ROCK_UPDOWN, "vswing -> rock 0x02");
  {
    HisenseCommand c = base;
    c.vswing = HISENSE_SWING_SWING;
    CHECK(cmd_byte(c, 32) == 0xC0, "vswing -> byte32 0x%02X exp 0xC0", cmd_byte(c, 32));
  }
  ...
  {
    HisenseCommand c = base;
    c.setpoint = matter_setpoint_to_c(2200);
    CHECK(cmd_byte(c, 19) == 0x2D, "setpt 22 -> byte19 0x%02X exp 0x2D", cmd_byte(c, 19));
  }
  ...
  {
    HisenseCommand c = base;
    c.mode = HISENSE_MODE_DRY;
    c.setpoint = 24;
    c.fan = HISENSE_FAN_HIGH;
  ...
  for (HisenseMode m : {HISENSE_MODE_COOL, HISENSE_MODE_HEAT, HISENSE_MODE_DRY, HISENSE_MODE_FAN, HISENSE_MODE_AUTO}) {
    HisenseMode back;
    bool ok = matter_mode_to_hisense(hisense_mode_to_matter(m), &back);
  ...
  for (int r = 0; r < 4; r++) {
    bool vv = (r & 2) != 0, hh = (r & 1) != 0, v2, h2;
    rock_to_swing(swing_to_rock(vv, hh), &v2, &h2);
  ...
  {
    int64_t w = hisense_active_power_mw(8) / 1000;
    CHECK(w >= 255 && w <= 280, "@55=8  -> %lldW exp ~268", (long long) w);
  }
  {
    int64_t w = hisense_active_power_mw(15) / 1000;
    CHECK(w >= 905 && w <= 965, "@55=15 -> %lldW exp ~934", (long long) w);
  }
  ...
  CHECK(hisense_active_power_mw(255) == HISENSE_POWER_MW_MAX, "@55=255 clamped to %lldmW",
        (long long) HISENSE_POWER_MW_MAX);
  ...
  {
    int64_t ma = hisense_active_current_ma(15, 220);
    CHECK(ma >= 4200 && ma <= 4700, "@55=15,220V -> %lldmA exp ~4470", (long long) ma);
  }
  ...
  {
    uint64_t acc = 0;
    hisense_energy_add(&acc, 1000000, 3600000);
    CHECK(hisense_energy_mwh(acc) == 1000000, "1kW*1h -> 1e6 mWh (1 kWh)");
  }
  {
    uint64_t acc = 0;
    hisense_energy_add(&acc, -5, 3600000);
    CHECK(acc == 0, "negative power clamped (import-only)");
  }
  ...
  printf("[#72 capability gating]\n");
  {
    ...
    f.power_save = true;
    CHECK(matter_gate_eco(&f), "eco: power_save=1 -> exposed");
    f.fan_mute = true;
    CHECK(matter_gate_quiet(&f), "quiet: fan_mute=1 -> exposed");
    f.power_display = 1;
    CHECK(matter_gate_display(&f), "display: power_display!=0 -> exposed");
    f.cool_heat = true;
    CHECK(matter_thermostat_featuremap(&f) == MATTER_THERMOSTAT_FEATUREMAP_FULL, "featuremap: cool_heat=1 -> full 35");
  ...
  printf("[#102 features bitmap roundtrip]\n");
  {
    ...
    samples[0].power_save = true;
    samples[0].cool_heat = true;
    ...
    samples[2].ext_valid = true;
    samples[2].fan_mute = true;
    samples[2].q_display = true;
    samples[2].demand_resp = 3;  // quiet, ext tier
    for (int i = 0; i < 3; i++) {
      uint32_t bm = hisense_features_to_bitmap32(&samples[i]);
      HisenseFeatures rt = {};
      hisense_features_from_bitmap32(bm, &rt);
      ...
      CHECK(matter_gate_display(&rt) == matter_gate_display(&samples[i]), "sample %d: display gate survives roundtrip",
            i);
      CHECK(matter_thermostat_featuremap(&rt) == matter_thermostat_featuremap(&samples[i]),
            "sample %d: featuremap survives roundtrip", i);
    }
    // first boot: nothing persisted -> word 0 -> valid=false -> gates permissive (show all)
    HisenseFeatures z = {};
    hisense_features_from_bitmap32(0, &z);
  ...
  printf("[fanmode echo ledger]\n");
  {
    ...
    for (unsigned i = 0; i < HISENSE_FAN_TABLE_LEN; i++) {
      ...
      CHECK(matter_echo_consume(&e, fm), "speed %u readback FanMode %u is an echo", k_hisense_fan_table[i].speed, fm);
    }
  }
