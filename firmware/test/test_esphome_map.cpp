...
int main() {
  ...
  printf("[mode]\n");
  CHECK(esphome_mode_to_hisense(ESPHOME_CLIMATE_MODE_COOL, &hm) && hm == HISENSE_MODE_COOL, "COOL->COOL");
  ...
  // ---- hvac_action ----
  printf("[action]\n");
  CHECK(hisense_to_climate_action(false, HISENSE_MODE_COOL, 0) == ESPHOME_CLIMATE_ACTION_OFF, "powered off -> OFF");
  ...
  CHECK(hisense_to_climate_action(true, HISENSE_MODE_FAN, 0) == ESPHOME_CLIMATE_ACTION_FAN, "fan mode -> FAN");
  ...
  CHECK(hisense_to_climate_swing(false, true) == ESPHOME_CLIMATE_SWING_HORIZONTAL, "h -> HORIZONTAL");
  ...
  for (int s = 0; s <= 3; s++) {
    ...
    CHECK(hisense_to_climate_swing(vs == HISENSE_SWING_SWING, hs == HISENSE_SWING_SWING) == s, "swing round-trip %d",
          s);
  }
  ...
  CHECK(hisense_fan_raw_to_esphome_index(HISENSE_FAN_RAW_AUTO) == ESPHOME_FAN_INDEX_AUTO, "raw 0x01 -> index 0");
  ...
  CHECK(
      esphome_fan_published_index(3) == 3 && esphome_fan_published_index(5) == 5 && esphome_fan_published_index(0) == 0,
      "other steps publish unchanged");
  ...
  for (uint8_t i = 1; i <= ESPHOME_FAN_INDEX_MAX; i++) {
    ...
    CHECK(cmd_byte(base, 16) == (uint8_t) (row->cmd * 2 + 1), "index %u -> command byte16 0x%02X", i, row->cmd * 2 + 1);
  ...
  for (uint8_t e : {(uint8_t) ESPHOME_CLIMATE_FAN_QUIET, (uint8_t) ESPHOME_CLIMATE_FAN_LOW,
    ...
    CHECK(cmd_byte(base, 16) == (uint8_t) (row->cmd * 2 + 1), "enum %u -> command byte16 0x%02X", e, row->cmd * 2 + 1);
  }
  ...
  printf("[shadow sync, #117]\n");
  {
    ...
    CHECK(hisense_build_command(&poisoned, buf, sizeof(buf)) == 0, "repro: poisoned shadow drops the COOL frame");
    ...
    s.setpoint = 26;
    CHECK(!esphome_sync_shadow_setpoint(hisense_f_to_c(24), true, &s) && s.setpoint == 26 && !s.fahrenheit,
        ...
        "both reported -> ECO (same precedence as the Matter sync)");
  {
    ...
    CHECK(esphome_feature_after_send(HISENSE_FEATURE_TURBO) == HISENSE_FEATURE_TURBO, "TURBO kept");
  }
  ...
  printf("[presets]\n");
  {
    static const char *const wrapper_names[] = {"none",          "eco",
                                                "quiet",         "turbo",
                                                "eco_quiet",     "sleep_general",
                                                "sleep_old",     "sleep_young",
                                                "sleep_kids",    "eco_sleep_general",
                                                "eco_sleep_old", "eco_sleep_young",
                                                "eco_sleep_kids"};
    ...
    static const char *const builtins[] = {"none", "home", "away", "boost", "comfort", "eco", "sleep", "activity"};
    for (unsigned i = ESPHOME_PRESET_FIRST_CUSTOM; i < ESPHOME_PRESET_COUNT; i++)
      for (unsigned b = 0; b < 8; b++)
        CHECK(strcasecmp(k_esphome_presets[i].name, builtins[b]) != 0, "custom %s is not built-in %s",
              k_esphome_presets[i].name, builtins[b]);
    ...
    const uint8_t all = ESPHOME_SUPPORT_ECO | ESPHOME_SUPPORT_QUIET | ESPHOME_SUPPORT_TURBO | ESPHOME_SUPPORT_SLEEP;
    ...
    CHECK(n == 1 && ops[0].kind == HISENSE_SPECIAL_OP_FEATURE && ops[0].value == HISENSE_FEATURE_ECO,
          "none -> eco: one ECO write");
    ...
    CHECK(n == 2 && ops[0].kind == HISENSE_SPECIAL_OP_FEATURE && ops[1].kind == HISENSE_SPECIAL_OP_MUTE &&
              ops[1].value == 1,
          "none -> eco_quiet: eco then mute");

    n = esphome_preset_plan(&none, 10, ops);  // eco_sleep_old
    CHECK(n == 2 && ops[0].kind == HISENSE_SPECIAL_OP_FEATURE && ops[1].kind == HISENSE_SPECIAL_OP_SLEEP &&
              ops[1].value == 2,
    ...
    CHECK(n == 2 && ops[0].value == HISENSE_FEATURE_ECO && ops[1].kind == HISENSE_SPECIAL_OP_SLEEP && ops[1].value == 2,
    ...
    CHECK(n == 2 && ops[0].value == HISENSE_FEATURE_ECO_OFF && ops[1].kind == HISENSE_SPECIAL_OP_SLEEP &&
              ops[1].value == 1,
    ...
    CHECK(
        n == 2 && ops[0].kind == HISENSE_SPECIAL_OP_MUTE && ops[0].value == 0 && ops[1].value == HISENSE_FEATURE_TURBO,
        "eco_quiet -> turbo: mute off, then turbo");
    ...
    for (unsigned a = 0; a < sizeof(starts) / sizeof(starts[0]); a++) {
      for (uint8_t t = 0; t < ESPHOME_PRESET_COUNT; t++) {
        ...
        for (uint8_t i = 0; i < n; i++)
          hisense_special_apply(&cur, &ops[i]);
        CHECK(esphome_preset_detect(&cur, all) == t, "start %u -> %s lands", a, k_esphome_presets[t].name);
      }
    }

    // fan pinning
    CHECK(esphome_fan_request_allowed(&none, 4), "free fan: any speed");
    CHECK(!esphome_fan_request_allowed(&turbo, 4) && esphome_fan_request_allowed(&turbo, 6), "turbo pins high");
